...
def page_prediction_body():
    ...
    st.info(
        "Enter the car's specifications below to predict its price using our "
        "trained ExtraTreesRegressor model (Test R² = 0.911)."
    )
    ...
    with col1:
        enginesize = st.number_input("Engine size", min_value=61, max_value=326, value=120)
        horsepower = st.number_input("Horsepower", min_value=48, max_value=288, value=100)
    with col2:
        curbweight = st.number_input("Curb weight", min_value=1488, max_value=4066, value=2400)
        cylindernumber = st.selectbox("Cylinder number", [2, 3, 4, 5, 6, 8, 12], index=2)
    with col3:
        citympg = st.number_input("City MPG", min_value=13, max_value=49, value=25)
    ...
    with col4:
        ...
        carwidth = st.number_input("Car width", min_value=60.3, max_value=72.3, value=65.0)
    with col5:
        carlength = st.number_input("Car length", min_value=141.1, max_value=208.1, value=175.0)
        doornumber = st.selectbox("Door number", [2, 4], index=1)
    with col6:
        drivewheel = st.selectbox("Drive wheel", ['fwd', 'rwd', '4wd'])
        wheelbase = st.number_input("Wheelbase", min_value=86.6, max_value=120.9, value=98.0)
    ...
    with col10:
        boreratio = st.number_input("Bore ratio", min_value=2.54, max_value=3.94, value=3.3)
        stroke = st.number_input("Stroke", min_value=2.07, max_value=4.17, value=3.3)
    with col11:
        compressionratio = st.number_input(
            "Compression ratio", min_value=7.0, max_value=23.0, value=9.0)
        peakrpm = st.number_input("Peak RPM", min_value=4150, max_value=6600, value=5200)
